  ...
  describe "playing the game" do
    context "with correct guesses" do
      ...
      it "submits three correct guesses and understands when the game is finished" do
        expect{ NewGuess.new(game, "A").call.save }.to change{ Guess.count }.by 1
        expect(Guess.last.correct_guess?("A")).to be true
        expect(GamePresenter.new(game).update_board).to eq(["_", "A", "_", "A", "_", "A"])
        ...
        expect(GamePresenter.new(game).update_board).to eq(["B", "A", "_", "A", "_", "A"])
        expect(game.guessed_letters).to eq(["A", "B"])

        expect{ NewGuess.new(game, "N").call.save }.to change{ Guess.count }.by 1
        expect(Guess.last.correct_guess?("N")).to be true
        expect(GamePresenter.new(game).update_board).to eq(["B", "A", "N", "A", "N", "A"])
    ...
    context "with incorrect guesses" do
      it "plays and loses the game" do
        ...
        expect{ NewGuess.new(game, "Z").call.save }.to change{ Guess.count }.by 1
